...
در اینجا میخوام کاری کنم که هر وقت دستور `exit` رو نوشتم اوبونتو رو ببنده و بعلاوه اون این فایل `reset.sh` رو هم اجرا کنه
...
```
alias "exit=source /path_to/reset.sh && exit"
```
</div>
...
خب حالا هر وقتی که من دستور `exit` رو بنویسم هم اوبونتو رو میبنده و هم تمام چیزها رو ریست میکنه و اینطوری مشکل من برای همیشه حل میشه

> توجه داشته باشید که هر وقتی که میخواهید اوبونتو رو به کل ببندید و دیگه سیستم رو خاموش و یا ری استارت کنید از دستور exit استفاده کنید شاید در ابتدا فراموش کنید اما با تکرار و تمرین به این دستور عادت میکنید
</div>